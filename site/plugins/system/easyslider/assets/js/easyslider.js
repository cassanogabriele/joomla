void function ( exports, $, _, Backbone ) {
	...
	var BackgroundView = View.extend({
        ...
        resetBackgroundPos: function(){
            if ( this.model.get('kenburn.enable') ) {
                var data = this.model.toJSON();
                //data.position = _.parseBackgroundPos(data.position, this.$el);
                //data.size = _.parseBackgroundPos(data.size, this.$el);
                //console.log('reset',data.position)

                this.$el.css({
                    transition: 'none',
                    'background-position': data.position,
                    /*'background-position-x': data.position[0] + 'px',
                    'background-position-y': data.position[1] + 'px',*/
                    'transform': 'scale(1)',
